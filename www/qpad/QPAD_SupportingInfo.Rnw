...
\begin{itemize}
  ...
  \item \code{BAM\_QPAD\_functions\_20130226.R}: sourceable text file with 
...
of the \pkg{detect} \R\ package. This function allows the user to select
...
<<pmodel,cache=TRUE,fig.keep='none'>>=
pmodel <- function() {
    for (i in 1:n) {
        Y[i] ~ dpois(lam[i])
