...
\title{dcpar: parallel computing and data cloning with JAGS (???)}
\Plaintitle{}
\Shorttitle{}
...
res1 <- lapply(k, function(z)
    timerfitfun(parallel = FALSE,
        dclone(datp, z, multiply = "n", unchanged = "np"),
        c("beta", "log.sigma"), custommodel(glmm.model, 4),
...
data.frame(n.clones=k, proc.time=pt1, percent=pt1/pt1[1])
...
data.frame(n.clones=k, proc.time=pt1, proc.time.par=pt2, percent=pt2/pt1)
...
t0 <- proc.time()
res3 <- dc.fit(datp, c("beta", "log.sigma"), custommodel(glmm.model, 4),
...
Sys.sleep(5)
t0 <- proc.time()
res4 <- dc.parfit(cl, datp, c("beta", "log.sigma"), custommodel(glmm.model, 4),
    n.clones = k, multiply = "n", unchanged = "np",
    n.update = n.update, n.iter = n.iter)
attr(res4, "timer") <- (proc.time() - t0)[3] / 60

t0 <- proc.time()
modb2 <- dc.parfit(cl, datb, c("beta", "log.sigma"), custommodel(glmm.model, 3),
    n.clones = k, multiply = "n", unchanged = "np",
    n.update = n.update, n.iter = n.iter)
tb2 <- proc.time() - t0
...
modp1 <- dc.fit(datp, c("beta", "log.sigma"), custommodel(glmm.model, 4),
    n.clones = k, multiply = "n", unchanged = "np",
    n.update = n.update, n.iter = n.iter)
tp1 <- proc.time() - t0
...
modp1 <- dc.fit(datb, c("beta", "log.sigma"), custommodel(glmm.model, 3),
    n.clones = k, multiply = "n", unchanged = "np",
    n.update = n.update, n.iter = n.iter)
tb1 <- proc.time() - t0
...
t0 <- proc.time()
modp2 <- dc.parfit(cl, datp, c("beta", "log.sigma"), custommodel(glmm.model, 4),
    n.clones = k, multiply = "n", unchanged = "np",
    n.update = n.update, n.iter = n.iter)
tp2 <- proc.time() - t0
