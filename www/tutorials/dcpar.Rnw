...
dat <- list(Y = Yp, X = X, n = n, np = NCOL(X))
...
t0 <- proc.time()
res3 <- dc.fit(dat, c("beta", "log.sigma"), custommodel(glmm.model, 4),
...
Sys.sleep(5)
t0 <- proc.time()
res4 <- dc.parfit(cl, dat, c("beta", "log.sigma"), custommodel(glmm.model, 4),
    n.clones = k, multiply = "n", unchanged = "np",
    n.update = n.update, n.iter = n.iter)
