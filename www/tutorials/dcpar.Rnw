...
The \pkg{dclone} R package \citep{Solymos2009} aims to provide low level functionality to easily implement 
more specific higher level procedures based on data cloning for users familiar with the Bayesian methodology.
The \pkg{dcpar} extends the \pkg{dclone} for parallel computations building on the infrastructure provided by the \pkg{snow} package (cit here). This paper I demonstrate how computing time can improve by using \pkg{dcpar}, and how can the parallelization effectively been optimized. The \pkg{dcpar} package currently
...
The first type of parallelization can be addressed by the \code{jags.parfit} function (parallelized version of \code{jags.fit}), while the second type can be addressed by the \code{dc.parfit} function (parallelized version of \code{dc.fit}).
...
@
Processing time (column \code{time}) increased almost linearly with $k$ (with and without parallelization, too; cf.~Fig.~\ref{fig1}), columns {rel.change} indicate change relative to $k=1$:
<<>>=
...
\begin{figure}[tb]
\begin{center}
<<fig1,echo=false,fig=true,width=7,height=7>>=
plot(k, pt1, xlab="Number of clones", ylab="Processing time (min)", ylim=range(pt1, pt2), cex=1.5, cex.lab=1.5, type="b")
points(k, pt2, col="red", cex=1.5, type="b")
@
\end{center}
\caption{Processing times of fitted model corresponding to different number of clones in the data set without (black) and with (red) parallelization.}
  \label{fig1}
\end{figure}

\section{Partitioning the problem into parallel subsets}

How to optimize (Fig.~\ref{fig2})?
<<>>=
clusterSize(pt1)
@
...
\begin{figure}[tb]
\begin{center}
<<fig1,echo=false,fig=true,width=9,height=3>>=
clusterSize(pt1)
...
Evaluate proc time:
\Sexpr{round(attr(res3, "timer"), 2)}

\Sexpr{round(attr(res4, "timer"), 2)}

(\Sexpr{round(100*attr(res4, "timer")/attr(res3, "timer"),2)} \%)
...
## (A) Poisson models, different k, jags.fit
## see that processing time is increasing linearly
...
## (B) Poisson models, different k, jags.parfit
## to compare with jags.fit results
...
lapply(res1, gelman.diag)
lapply(res2, gelman.diag)
...
data.frame(n.clones=k, proc.time=pt1, percent=100*pt1/pt1[1])

plot(k, pt1, xlab="Number of clones", ylab="Processing time (min)", ylim=range(pt1, pt2))
abline(0, pt1[1],lty=2)
abline(lm(pt1 ~ k - 1),lty=1)

points(k, pt2, col="red")
abline(0, pt2[1], lty=2, col="red")
abline(lm(pt2 ~ k), lty=1, col="red")
...
data.frame(n.clones=k, proc.time=pt1, proc.time.par=pt2, percent=100*pt2/pt1)


expsigma <- function(x)
    mcmcapply(x[,"log.sigma"], exp)

sapply(res1, function(z) mean(expsigma(z)))
sapply(res2, function(z) mean(expsigma(z)))
sapply(res1, function(z) sd(expsigma(z)))
sapply(res2, function(z) sd(expsigma(z)))

attr(res3, "timer")

attr(res4, "timer")

plot.dctable(dctable(res4))
plot.dctable(dctable(res1[[1]],res1[[2]],res1[[3]],res1[[4]],res1[[5]]))
plot.dctable(dctable(res2[[1]],res2[[2]],res2[[3]],res2[[4]],res2[[5]]))
