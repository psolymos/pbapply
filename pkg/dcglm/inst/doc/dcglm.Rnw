...
in \proglang{JAGS} is stricter than the usual numerical precision in \R, consequently we ensure that
...
\begin{enumerate}
  ...
  \item \code{prec| and \code{coefs} is nneded for the data specification.
...
The code to produce the graphs are as follows:
\begin{figure}[tbh]
\begin{center}
<<predfig,fig=true,echo=true,width=7,height=7>>=
