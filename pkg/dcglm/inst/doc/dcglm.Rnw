...
\Abstract{
  This tutorial package to demonstrates the 
  capabilities of data cloning algorithm via 
  the infrastructure provided by the \pkg{dclone} package.
  The functions developed here reproduce main features of the \code{glm}
...
This tutorial, we develop high level functions to reproduce the \code{glm} base function of \proglang{R} by using
...
str(dat1)
@
Now let's clone the data set (note that \code{n} should be multiplied, while
...
summary(mod1)
@
Let's compare this \code{mcmc.list} (more accurately an \code{mcmc.list.dc}) object with the \code{glm} results:
<<>>=
...
Here is the \proglang{JAGS} model for the Binomial (Bernoulli, because only one trial) case:
<<>>=
glm.bin <- function() {
    for (i in 1:n) {
        Y[i] ~ dbin(p[i], k)
...
The \code{custommodel} function enables us to reuse the same \proglang{JAGS} model
...
which line(s) should be excluded:
<<>>=
...
as valid \proglang{R} objects or functions. This is, however, a requisite of
building a valid \proglang{R} package that that passes \code{R CMD check}.
A way to overcome this is to define fake objects as e.g.~\code{inprod <- function() NULL},
...
Here is our main function for the data cloning based estimating procedure
...
Let'g go through this function step-by-step as pseudo-code:
...
\begin{enumerate}
  ...
  \item \code{formula[[2]] <- NULL} removes the left-hand-side from the formula (that's why
    we keep a copy of it named as \code{formula.orig}).
  ...
  \item The the Bayesian (\code{dat}) and data cloned (\code{dcdat}) data representation, and \code{model} 
    depends on the \code{family} argument.
...
\footnote{Note that these are identical to the \code{coef.default} and \code{fitted.default}
functions, but shown here for didactic purposes.}:
<<>>=
...
prval <- jags.fit(prdat, "z", custommodel(glm.pred, c(4,6)), 
...
\begin{enumerate}
  ...
  \item \code{prec} and \code{coefs} are needed for the data specification.
  \item If \code{newdata} is \code{NULL}, we use the extracted design matrix
    of our fitted model (\code{object}). Else, we create the design matrix
  ...
  \item \code{prdat} is the data, \code{prmod} is the fitted MCMC object (we use \code{k}
  only in case of the Bionimal family, otherwise \pkg{rjags} warns us that \code{k} is unused
...
Follow this workflow for your own model and estimating procedure, then edit the files (read the \emph{Writing \proglang{R} Extensions} manual) in the package directory, run \code{R CMD check} and \code{R CMD build}, distribute your package, and go out for a walk!
