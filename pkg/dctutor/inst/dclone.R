...
###################################################
library(dclone)
setwd("c:/svn/abmiserver/dclone-paper/final") ## this needs to be edited
load("resultsOven.Rdata")
load("resultsPois.Rdata")
load("resultsBevholt.Rdata")
load("NN1.Rdata")
options(width=50)
options(digits=3)
source("setup.R") ## for hard coding par(las=1)
...
###################################################
dcdiag(nnmod)
vars <- mcmcapply(nnmod[,c("sigma","tau")], array)^2
sigma^2 + tau^2
